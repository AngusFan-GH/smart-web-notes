// Background Script - 指向新的 src 目录
export default defineBackground(() => {
  console.log("Smart Web Notes Background Script Started");

  // 导入新的 background script
  import("../src/background/index.ts").catch(console.error);
  ...
  async function handleAnswerGeneration(
    ...
    try {
      ...
      console.log("收到LLM响应:", response.substring(0, 100) + "...");

      // 保存答案
      currentAnswers[tabId] = response;
      completedAnswers[tabId] = response;
